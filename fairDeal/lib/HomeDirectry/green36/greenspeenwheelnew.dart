...
import 'package:fairdeal/constant/bada_rollet_dependency.dart';
...
class greenspeenwheelnew extends StatefulWidget {
  final double widthc;
  ...
  greenspeenwheelnew({ required this.widthc,
    required this.heightc,
    required this.pad,
    required this.result,
    required this.time,
    required this.gnno, required this.fitime,});

  @override
  _greenspeenwheelnewState createState() => _greenspeenwheelnewState();
}

class _greenspeenwheelnewState extends State<greenspeenwheelnew> with SingleTickerProviderStateMixin{
  ...
  void initState() {
    ...
    _timer = Timer.periodic(Duration(seconds: 1), (timer) {
      ...
      if(_clockModel.second==58){
        wheelSpinController.stopWheel(widget.time-1);
      }
...
int num=37;
  ...
  tripleapis() async {
    ...
    wheelSpinController.stopWheel(win-1);
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
            image: DecorationImage(
              image: AssetImage('assets/chakra123.png'),
            )
        ),
        child: WheelSpin(
